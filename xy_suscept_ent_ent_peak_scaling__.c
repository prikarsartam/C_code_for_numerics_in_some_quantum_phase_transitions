...
long double xy_size_vs_peak_distance_from_criticality(int n) {
...
void calculate_and_store_data(const int* size_list, int size_count, const char* filename) {
    ...
    for (int i = 0; i < size_count; i++) {
        ...
        long double distance = xy_size_vs_peak_distance_from_criticality(size);
        fprintf(file, "%d,%.30Lf\n", size, distance); 

        printf("| gamma_c(%d) - gamma_c(Infty) |  = %.30Lf\n", size, distance);
    }
...
int size_list[] = {
    ...
    calculate_and_store_data(size_list, size_count, "data/xy_suscept_peak_data__30072024_0138__inLongDouble.csv");
