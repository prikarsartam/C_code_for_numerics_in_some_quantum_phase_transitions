...
long double tfim_size_vs_peak_height(int n) {
    long double h_critical = optimize_g11(n);
    return calculate_g11(h_critical, 1.0, n);
}
...
void calculate_and_store_data(const int* size_list, int size_count, const char* filename) {
    ...
    fprintf(file, "System_Size,Peak_Heights\n");
    
    printf("\n\n Calculating the peak heights\n\n");
    
    for (int i = 0; i < size_count; i++) {
        int size = size_list[i];
        long double peak_height = tfim_size_vs_peak_height(size);
        fprintf(file, "%.24Lf,%.24Lf\n", (long double)size, peak_height);

        printf("Max(Suscept_%d) = %.24Lf\n", size, peak_height);
    }
...
int size_list[] = {
    ...
    748, 800, 850, 900, 950, 1000, 1250, 1400, 1650, 1800, 2000
};
    ...
    calculate_and_store_data(size_list, size_count, "tfim_suscept_peakHeight_data__28072024_1436__inLongDouble.csv");
